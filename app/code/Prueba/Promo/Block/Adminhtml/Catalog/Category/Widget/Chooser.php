...
namespace Prueba\Promo\Block\Adminhtml\Catalog\Category\Widget;
...
class Chooser extends \Magento\Catalog\Block\Adminhtml\Category\Widget\Chooser
{
    ...
    public function prepareElementHtml(\Magento\Framework\Data\Form\Element\AbstractElement $element)
    {
        ...
        $chooser = $this->getLayout()->createBlock('Magento\Widget\Block\Adminhtml\Widget\Chooser')
            ->setElement($element)->setConfig($this->getConfig())->setFieldsetId($this->getFieldsetId())
            ->setSourceUrl($sourceUrl)->setUniqId($uniqId);
        if ($element->getValue()) {
    ...
    public function getNodeClickListener()
    {
        ...
        if ($this->getUseMassaction()) {
            ...
            $chooserJsObjet = $this->getId();
            $js = 'function (node, e) {'.$chooserJsObjet.
                '.setElementValue(node.attributes.id);' .$chooserJsObjet. '.setElementLabel(node.test);'
                .$chooserJsObjet.'.close();}';
        }
        return $js;
    }
}
